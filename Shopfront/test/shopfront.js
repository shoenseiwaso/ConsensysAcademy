...
contract('Shopfront', function(accounts) {
  ...
  it("should add a valid Shopfront with Alice, Bob and Carol", function () {
    return contract.insertShopfront(
      u.alice.addr, 
      u.alice.name, 
      u.bob.addr, 
      u.bob.name, 
    ...
    .then(function(txn) {
      return contract.getShopfrontAtIndex(0);
    })
    .then(function(v) {
  ...
  it("should refuse to add a duplicate Shopfront (same 'from' address)", function () {
    return contract.insertShopfront(
      u.alice.addr, 
      u.alice.name, 
      u.bob.addr, 
  ...
  it("should add a valid Shopfront with Alice, Bob and Carol, then a second one with Bob, David and Emma, with an odd amount of wei", function () {
    return contract.insertShopfront(
      u.alice.addr, 
      u.alice.name, 
      u.bob.addr, 
    ...
    .then(function(txn) {
      return contract.getShopfrontAtIndex(1);
    })
    .then(function(v) {
      // compute expected balances, handling case where test value is an odd number
